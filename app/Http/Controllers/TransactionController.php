...
class TransactionController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('transactions.index', compact('transactions', 'members', 'statistics'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('cash_receipt')) {
            return response()->json([
                'success' => true,
                'message' => $transaction->cash_receipt ? '현금영수증이 발행완료로 변경되었습니다.' : '현금영수증이 미발행으로 변경되었습니다.',
                'cash_receipt' => $transaction->cash_receipt
            ]);
        }
    ...
    public function export(Request $request)
    {
        try {
            ...
            $spreadsheet->getProperties()
                ...
                ->setTitle('현금영수증 데이터')
                ->setSubject('현금영수증 데이터')
                ->setDescription('현금영수증 발행을 위한 데이터')
                ->setKeywords('현금영수증')
                ->setCategory('Finance');
            ...
            // 파일명 생성
            $filename = '현금영수증_' . now()->format('Y-m-d') . '.xls';
