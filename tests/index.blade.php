                    ...
                    <div class="tab-content h-100">
                        ...
                        <div class="tab-pane fade h-100" id="text-content" role="tabpanel">
                            <div class="d-flex justify-content-between align-items-center p-2 bg-light border-bottom sticky-top">
                                <span>보정서 항목정리</span>
                                <div>
                                    <button id="convert-to-text" class="btn btn-sm btn-primary me-1 d-none">텍스트 변환</button>
                                    <button id="format-text" class="btn btn-sm btn-success me-1 d-none">변환2</button>
                                    <button id="fetch-last-response" class="btn btn-sm btn-info me-1 d-none">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // 텍스트 포맷팅 버튼 (보정권고/명령 정리)
        formatTextBtn.addEventListener('click', formatLegalText);
...
\`\`\``;
        ...
        function loadFileList(page = 1, append = false) {
            ...
            // API 호출
            fetch(`{{ route('document-editor.files') }}?handler=${handlerFilter.value}&search=${searchInput.value}&page=${page}&per_page=20`)
                .then(response => response.json())
                .then(data => {
                    // 로딩 표시 제거
        ...
        function convertPDFToText() {
            if (!currentFilePath) {
                alert('변환할 PDF 파일을 먼저 선택해주세요.');
                return;
            }
            
            // 로딩 모달 표시
            document.getElementById('loading-message').textContent = 'PDF를 텍스트로 변환 중입니다...';
            loadingModal.show();
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    alert('PDF 변환 중 오류가 발생했습니다: ' + data.message);
                }
            })
            .catch(error => {
        ...
        // 텍스트 포맷팅 함수 (보정권고/명령 정리)
        function formatLegalText() {
            if (!textEditor.value.trim()) {
                alert('변환할 텍스트가 없습니다. 먼저 텍스트 변환을 실행해주세요.');
                return;
            }
            
            // 타임아웃 가능성 안내
            const isLongText = textEditor.value.length > 5000;
            if (isLongText) {
                const confirmMsg = '긴 텍스트는 처리 중 타임아웃이 발생할 수 있습니다. 이 경우 나중에 다시 시도해주세요. 계속하시겠습니까?';
            ...
            // 로딩 모달 표시
            document.getElementById('loading-message').textContent = '텍스트를 항목별로 정리하는 중입니다...';
            loadingModal.show();
            
            // 모든 텍스트는 일반 처리로 통일
            processSingleRequest(textEditor.value);
            
            // 10초 후에 '가져오기' 버튼 사용 안내 (로딩 중인 경우에만)
            setTimeout(() => {
                if (document.getElementById('loading-message').textContent.includes('처리 중') || 
                    document.getElementById('loading-message').textContent.includes('기다리는 중')) {
                    const fetchBtn = document.getElementById('fetch-last-response');
                    fetchBtn.classList.add('btn-blink');
                    ...
                    // 힌트 표시
                    document.getElementById('loading-message').textContent = 
                        '처리 시간이 길어지고 있습니다. 나중에 다시 시도해주세요.';
                }
            }, 10000);
        }
        
        // 단일 요청으로 텍스트 처리
        function processSingleRequest(text) {
            // LLM에 전송할 프롬프트
...
\`\`\``;
            ...
            document.getElementById('loading-message').textContent = 'API 요청을 처리 중입니다...';
            loadingModal.show();
            
            // legal-chat.send API를 이용하여 대화 저장 방식으로 처리
            fetch('{{ route("legal-chat.send") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            .then(data => {
                // 응답이 있으면 바로 적용
                if (data.answer) {
                    textEditor.value = data.answer;
                    loadingModal.hide();
                    return;
                }
                
                // 응답이 없거나 불완전하면 대화 ID로 결과 폴링
                if (data.conversation_id) {
                    // 폴링 시작 메시지
                    document.getElementById('loading-message').textContent = '응답을 기다리는 중입니다...';
                    
                    // 대화 내용을 폴링하여 완성된 응답을 가져옴
                    pollConversation(data.conversation_id);
                } else {
                    // 대화 ID가 없는 경우 오류 표시
        ...
        // 대화 폴링 함수
        function pollConversation(conversationId, attempts = 0) {
            // 최대 시도 횟수 (5분 동안 10초마다 = 약 30회)
            const maxAttempts = 30;
            
            if (attempts >= maxAttempts) {
                // 최대 시도 횟수 초과 시 모달 닫기 및 가져오기 버튼 깜빡임 효과 추가
                closeLoadingModalCompletely(); // 모달을 완전히 닫는 통합 함수 호출
            ...
            // 폴링 진행 메시지 업데이트
            document.getElementById('loading-message').textContent = 
                `응답을 기다리는 중입니다... (${attempts + 1}/${maxAttempts})`;
            ...
            fetch(`{{ url('legal-chat/conversation') }}/${conversationId}`)
                ...
                .then(data => {
                    if (data.messages && data.messages.length >= 2) {
                        ...
                        if (lastMessage.role === 'assistant' && lastMessage.content) {
                            // 응답을 텍스트 에디터에 설정
                            textEditor.value = lastMessage.content;
                            ...
                            // 가져오기 버튼의 깜빡임 효과 제거
                            const fetchBtn = document.getElementById('fetch-last-response');
                            fetchBtn.classList.remove('btn-blink');
                            
                            console.log('대화에서 응답을 성공적으로 가져왔습니다.');
                            return;
                        }
                    }
                    ...
                    // 응답이 아직 완료되지 않은 경우 10초 후에 다시 폴링
                    setTimeout(() => {
                        pollConversation(conversationId, attempts + 1);
                    }, 10000);
                })
                .catch(error => {
                    console.error('Error polling conversation:', error);
                    
                    // 오류 발생 시 모달 닫고 가져오기 버튼 깜빡이게 함
                    closeLoadingModalCompletely();
                    document.getElementById('loading-message').textContent = '처리 중...';
                    ...
                    alert('응답을 가져오는 중 오류가 발생했습니다. 나중에 다시 시도해주세요.');
                });
        }
...
`;
        ...
        // 텍스트 변환 버튼
        convertToTextBtn.addEventListener('click', convertPDFToText);
        ...
        // 가져오기 버튼
        document.getElementById('fetch-last-response').addEventListener('click', fetchLastResponse);
...
\`\`\``;
        ...
        function fetchLastResponse() {
            document.getElementById('loading-message').textContent = '마지막 대화 응답을 가져오는 중...';
            
            // bootstrap 모달 객체를 사용하여 명시적으로 모달 표시
            if (typeof loadingModal === 'object' && loadingModal !== null) {
                loadingModal.show();
            } else {
                // DOM 요소에서 직접 bootstrap 모달 객체 생성
                const modalElement = document.getElementById('loadingModal');
                if (modalElement) {
            ...
            fetch('{{ route("legal-chat.conversations") }}')
                ...
                .then(data => {
                    if (!data.conversations || data.conversations.length === 0) {
                        // 먼저 모달 닫기
                        closeLoadingModalCompletely();
                        throw new Error('대화 목록이 비어 있습니다. 먼저 텍스트 변환을 수행해주세요.');
                    }
                ...
                .then(data => {
                    if (!data.messages || data.messages.length < 2) {
                        // 먼저 모달 닫기
                        closeLoadingModalCompletely();
                        throw new Error('대화에서 응답을 찾을 수 없습니다.');
                    }
                    ...
                    if (!lastAiMessage || !lastAiMessage.content) {
                        // 먼저 모달 닫기
                        closeLoadingModalCompletely();
                        throw new Error('대화에서 유효한 AI 응답을 찾을 수 없습니다.');
                    }
                    
                    // 응답을 텍스트 에디터에 설정
                    textEditor.value = lastAiMessage.content;
                    
                    // 가져오기 버튼의 깜빡임 효과 제거
                    const fetchBtn = document.getElementById('fetch-last-response');
                    fetchBtn.classList.remove('btn-blink');
                    
                    // 먼저 모달 닫기 (중요: alert 전에 실행)
                    closeLoadingModalCompletely();
                    
                    // 응답 시간 계산 및 로깅
                    const responseTime = new Date().getTime() - startTime;
                    console.log(`가져오기 작업 완료: ${responseTime}ms 소요`);
                    
                    // 약간의 지연 후 알림 표시 (모달이 확실히 닫힌 후)
                    setTimeout(() => {
                        // 성공 메시지
                        alert('마지막 대화에서 응답을 성공적으로 가져왔습니다.');
                    }, 300);
                })
                .catch(error => {
                    console.error('Error fetching last response:', error);
                    ...
                    // 약간의 지연 후 알림 표시 (모달이 확실히 닫힌 후)
                    setTimeout(() => {
                        alert('응답을 가져오는 중 오류가 발생했습니다: ' + error.message);
                    }, 300);
                });
        }
    });
