...
from llm_client import generate_response, generate_general_response
from chat_manager import ChatManager
from utils import sanitize_user_input, format_response_for_frontend, logger

app = FastAPI(title="법무법인 ERP 법률 AI 챗봇 API")
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"서버 시작: {config.API_HOST}:{config.API_PORT}")
    uvicorn.run("app:app", host=config.API_HOST, port=config.API_PORT, reload=config.DEBUG_MODE) 