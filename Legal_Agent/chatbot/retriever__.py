...
import pinecone  # from pinecone import Pinecone 대신 모듈 전체를 import
...
# Pinecone 초기화 - 서버 환경에 맞게 수정
try:
    # 이전 버전 방식만 사용
    pinecone.init(api_key=config.PINECONE_API_KEY, environment=config.PINECONE_ENVIRONMENT)
    index = pinecone.Index(config.PINECONE_INDEX_NAME)
    pinecone_version = "old"
    logger.info("Pinecone API 초기화 성공")
...
def retrieve_relevant_articles(query: str, top_k: int = 3) -> List[Dict[str, Any]]:
    ...
    # Pinecone에서 벡터 검색 (구버전 API 방식만 사용)
