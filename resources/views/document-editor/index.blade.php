...
2. 각 보정 항목에 대해 보정 여부를 판단합니다 (완료됐으면 O, 아직 안됐으면 빈문자열).
3. 추가보정 예정일자가 언급된 경우 이를 (예시 : 2025-05-30표시합니다.
4. 미보정 사유가 있다면 이를 비고란에 요약하여 표시합니다.

두 번째 표: 필요 서류 목록
1. 텍스트에서 제출을 요구하는 모든 서류 목록을 추출합니다.
2. 각 서류별로 제출 여부(제출됐으면 O, 아직 안됐으면 빈문자열)를 표시합니다.
...
\`\`\`html
<!-- 첫 번째 표: 보정권고 요약표 -->
<h5 class="mb-3">보정권고에 요청된 보정항목</h5>
<table class="table table-bordered table-striped">
  <thead class="table-primary">
    <tr>
      ...
      <th style="width: 10%; text-align: center; vertical-align: middle;">보정여부</th>
      <th style="width: 15%; text-align: center; vertical-align: middle;">추가보정<br>(예정일자)</th>
      <th style="width: 20%; text-align: center; vertical-align: middle;">비고<br>(미보정사유)</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>1</td>
      <td>[보정권고 항목 1 요약]</td>
      <td class="text-center">[O 또는 빈문자열]</td>
...
<div class="mt-4">
  ...
  <table class="table table-bordered table-striped">
    <thead class="table-secondary">
      <tr>
        <th style="width: 5%; text-align: center; vertical-align: middle;">번호</th>
        <th style="width: 60%; text-align: center; vertical-align: middle;">필요서류</th>
        <th style="width: 15%; text-align: center; vertical-align: middle;">제출여부</th>
        <th style="width: 20%; text-align: center; vertical-align: middle;">제출예정일</th>
      </tr>
    </thead>
    <tbody>
      <tr>
...
3. 보정 여부가 'O'인 경우 text-success 클래스를 추가해주세요.
...
6. 보정여부 및 제출여부는 텍스트에서 '제출합니다', '반영하였습니다', '수정하였습니다' 등 긍정적인 표현을 통해 판단하고 제출하지 않았지만 소명을 통해 법원의 양해를 구하는 내용도 O로 판단합니다. 다만, 추후 제출의 의사를 명확히 하면 제출하지 않은 것으로 판단합니다.
7. 예정일자가 언급된 경우 그대로 표기합니다. (예: "2025-05-30")
8. 미보정 사유는 텍스트에서 미보정 이유가 명확하게 설명된 경우에만 표기합니다.
...
2. 각 항목(숫자, 기호로 구분된)을 명확히 구분하고 적절히 여유있는 행간을 두어 가독성을 높여주세요. 중첩된 목록(예: 가, 나, 다 또는 ①, ②, ③)도 모두 포함하고 형식을 유지해야 합니다.
3. 내용 자체는 절대 수정하지 말고, 원문 그대로 유지하면서 형식만 정리해주세요.
4. 항목 번호 체계를 유지하되, 각 항목 사이에는 줄바꿈과 여유있는 여백을 추가해 구분해주세요.
5. 다음과 같은 형식적인 내용은 모두 제외해주세요:
   - 문서 상단/하단의 법원명, 사건번호, 신청인 정보, 날짜, 담당자 정보 등 헤더/푸터 정보
   - '기재례'와 같이 기재를 어떤 식으로 해야 하는지 안내하는 형식적인 내용
   - 참고사항, 주의사항, 요청사항에 해당하는 내용
   - 자료제출목록 등 체크리스트 양식 등
   - 형식적인 문서 구분선(예: "인천지방법원")
   - "채무자는 이 보정권고를 송달받은 날로부터 14일 이내에 아래사항을 보정하여 주시기 바랍니다" 같은 상투적 안내문구
   - "아래", "등본입니다" 같은 형식적 구분자
6. 오직 사용자가 실제로 해야 할 보정사항만 깔끔하게 정리해주세요.
7. 모든 항목을 포함할 수 있도록 내용을 압축하지 말고 원본 그대로 유지하세요.
8. 위 항목정리가 모두 끝난 후에는 문서의 하단에 첨부서류(가운데 정렬)라는 제목을 두고 그 밑에 순번을 붙여 이 보정권고(보정명령)에서 제출을 요구하는 모든 서류목록을 추가합니다. 
9. 첨부서류 목록은 원본 텍스트의 순서를 따르고 줄바꿈 하여 표시하되 행간을 띄우지 않습니다.
10. 중복되는 서류는 하나만 남깁니다.
